    ...
    >
      <div className="max-w-[1100px] mx-auto">
        ...
        <h2
          ...
          style={{ fontSize: "clamp(28px,4vw,42px)", color: "var(--warm-white)", lineHeight: 1.1 }}
        >
          Pre-design &amp; construction analysis
        </h2>
        <p className="text-[15px] leading-[1.7] mb-12" style={{ color: "rgba(245,242,237,0.6)", maxWidth: "560px" }}>
          Strategic guidance from someone who&apos;s actually built it — across every trade.
        </p>

        <div className="grid grid-cols-1 sm:grid-cols-3 gap-px" style={{ background: "var(--slate-700)" }}>
          {consultationItems.map(({ title, body }) => (
            ...
            >
              ...
              />
              <h4 className="font-serif text-lg mb-3" style={{ color: "var(--warm-white)" }}>
                {title}
              </h4>
              <p className="text-sm leading-[1.75]" style={{ color: "rgba(245,242,237,0.55)" }}>
                {body}
              </p>
            </div>
          ))}
        ...
        <p className="mt-8 text-[13px]" style={{ color: "rgba(245,242,237,0.4)", borderLeft: "2px solid var(--accent-gold)", paddingLeft: "14px" }}>
